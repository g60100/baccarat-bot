...
def build_caption_text(user_id, is_analyzing=False):
    ...
    elif recommendation: rec_text = f"\n\n👇 *AI 추천 참조* 👇\n{'🔴' if recommendation == 'Banker' else '🔵'} *{escape_markdown(recommendation + '에 베팅을 참조하세요.')}*"
    
    title = escape_markdown("ZENTRA가 개발한 AI 분석기로 베팅에 참조 하세요. 결정은 본인이 하며, 결정의 결과도 본인에게 있습니다."); subtitle = escape_markdown("승리한 쪽의 버튼을 눌러 기록을 누적하세요.")
